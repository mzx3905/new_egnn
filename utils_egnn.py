...
class GraphDataset(InMemoryDataset):
    ...
    def process(self, graphs_dict):
        ...
        for key in graphs_dict:
            size, features, edge_index,coords, edge_weight= graphs_dict[key]

            GCNData = DATA.Data(x=torch.Tensor(features), edge_index=torch.LongTensor(edge_index).transpose(1, 0),edge_weight=torch.Tensor(edge_weight),coords=torch.Tensor(coords))
...
def model_evaluate(Y, P,dataset):
    ...
    return (get_mse(Y, P),
            ...
            get_rm2(Y, P),
            get_pearson(Y, P),
            get_aupr(Y, P, threshold=thresholds[dataset]))